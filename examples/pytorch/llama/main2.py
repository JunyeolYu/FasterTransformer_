...
def main():
    ...
    with NumpySocket() as s:
        ...
        final_reqs = s.recv()
        print("reqs recieved, i'm rank {}".format(rank))
    ...
    final_reqs2 = engineering_dataset(validation_zeroshot, tokenizer)

    for i in range(10):
        current = final_reqs2[len(final_reqs2) -1 - i]
        current_socket = final_reqs[len(final_reqs2) -1 - i]
        print(final_reqs[i][0])
        print("\n\n")
        print(final_reqs2[i][0])
    exit()
    ...
    print("After model loading, before eval {}".format(start_eval - start_model))
